  ...
  if (!empty($_POST['order_product_number'])) {
    ...
    for ($i = 0; $i < $count; $i++) {
      ...
      if ($original_qty !== null && $original_undelivered !== null && $product_name !== null && $delivery_qty > 0 && $delivery_qty <= $original_undelivered) {
        $new_delivered = $original_qty + $delivery_qty; // 納品済み累計
        $new_undelivered = $original_undelivered - $delivery_qty; // 残り未納品
    ...
    window.addEventListener('DOMContentLoaded', function() {
      document.querySelectorAll('tbody tr').forEach(tr => {
        ...
        if (qtyInput && undelivered) {
          ...
          if (!isNaN(qtyVal) && qtyVal > 0) updateUnDelivered(qtyInput);
        }
      });
    });
  </script>
